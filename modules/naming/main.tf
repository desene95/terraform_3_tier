...
locals {
  ...
  az = {
    recovery_services_vault = {
      name        = substr(join("", compact([local.prefix_safe, "rsvault", local.suffix_safe])), 0, 63)
      name_unique = substr(join("", compact([local.prefix_safe, "rsvault", local.suffix_unique_safe])), 0, 63)
      dashes      = false
      slug        = "rsvault"
      min_length  = 2
      max_length  = 50
    ...
    backup_policy_vm = {
      name        = substr(join("", compact([local.prefix_safe, "bkupvm", local.suffix_safe])), 0, 63)
      name_unique = substr(join("", compact([local.prefix_safe, "bkupvm", local.suffix_unique_safe])), 0, 63)
      dashes      = false
      slug        = "bkupvm"
      min_length  = 3
      max_length  = 150
    ...
    firewall_policy = {
      name        = substr(join("-", compact([local.prefix, "pol", local.suffix])), 0, 80)
      name_unique = substr(join("-", compact([local.prefix, "pol", local.suffix_unique])), 0, 80)
      dashes      = true
      slug        = "pol"
      min_length  = 1
      max_length  = 80
    ...
    function_app = {
      name        = substr(join("-", compact([local.prefix, "afa", local.suffix])), 0, 32)
      name_unique = substr(join("-", compact([local.prefix, "afa", local.suffix_unique])), 0, 32)
      dashes      = true
      slug        = "fa"
      min_length  = 1
      max_length  = 60
    ...
    app_service_plan = {
      name        = substr(join("-", compact([local.prefix, "asp", local.suffix])), 0, 60)
      name_unique = substr(join("-", compact([local.prefix, "asp", local.suffix_unique])), 0, 60)
      dashes      = true
      slug        = "asp"
      min_length  = 1
      max_length  = 60
